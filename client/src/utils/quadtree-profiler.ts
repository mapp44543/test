...
export class QuadtreeProfiler {
  ...
  compareWithBaseline() {
    ...
    const avgTimeStr = statsMap['Avg Hit Detection Time (ms)'] || '0';
    const avgCandidatesStr = statsMap['Avg Candidates Checked'] || '0';
    
    const avgTime = typeof avgTimeStr === 'string' ? parseFloat(avgTimeStr) : avgTimeStr;
    const avgCandidates = typeof avgCandidatesStr === 'string' ? parseFloat(avgCandidatesStr) : avgCandidatesStr;
    
    console.log(`Average time: ${avgTime.toFixed(3)}ms`);
    console.log(`Candidates checked: ${avgCandidates.toFixed(1)}`);
